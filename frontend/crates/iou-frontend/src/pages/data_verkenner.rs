...
use crate::components::{
    build_maplibre_flevoland_init_script, build_restore_state_script, catalog_layers, GeoFilterPanel,
    Header, LayerControl3D, Panel, load_flevoland_layers_config,
};
...
/// MapLibre initialization: 2D catalog from `assets/geodata/flevoland_layers.json` (no 3D buildings).
fn get_map3d_init_script() -> String {
    let cfg = load_flevoland_layers_config();
    build_maplibre_flevoland_init_script(&cfg)
}
