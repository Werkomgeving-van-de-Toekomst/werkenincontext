...
use crate::components::layer_control_3d::{FlevolandLayersConfig, GeoJsonLayer, LayerType};

/// MapLibre init script for the Data Verkenner: 2D view, GeoJSON layers from catalog (no 3D buildings).
pub fn build_maplibre_flevoland_init_script(config: &FlevolandLayersConfig) -> String {
    let cfg_json =
        serde_json::to_string(config).expect("FlevolandLayersConfig must serialize to JSON");
    format!(
        r#"
    (function() {{
        const MAP_CFG = {cfg_json};
        ...
        function initMap2D() {{
            ...
            try {{
                var map = new maplibregl.Map({{
                    container: 'map',
                    style: MAP_CFG.style_url,
                    center: MAP_CFG.map.center,
                    zoom: MAP_CFG.map.zoom,
                    pitch: MAP_CFG.map.pitch,
                    bearing: MAP_CFG.map.bearing,
                    antialias: true
                }});
                ...
                map.on('load', function() {{
                    MAP_CFG.layers.forEach(function(layer) {{
                        if (layer.kind === 'raster') {{
                            ...
                            if (layer.minzoom != null) src.minzoom = layer.minzoom;
                            if (layer.maxzoom != null) src.maxzoom = layer.maxzoom;
                            if (layer.bounds != null) src.bounds = layer.bounds;
        ...
        waitForElement('map', function() {{ initMap2D(); }});
    }})();
    "#,
        cfg_json = cfg_json
    )
...
/// Parameters voor attribuutfilter → nieuwe GeoJSON-laag (Wikiwijs/EduGIS-workflow).
#[derive(Debug, Clone, Serialize)]
pub struct GeoFilterParams {
    pub url: String,
...
/// Voert filter client-side uit: fetch GeoJSON, filter features, `addSource`/`addLayer` op `window.map_map`.
pub fn build_apply_geo_filter_script(p: &GeoFilterParams) -> String {
    assert!(is_valid_container_id(&p.result_id), "Invalid result_id");
    let json = serde_json::to_string(p).expect("GeoFilterParams serializes");
    format!(
        r#"
        (function() {{
            const p = {};
            ...
            if (!map) {{
                console.error('Map niet gevonden');
                return;
            }}
            fetch(p.url)
                .then(function(r) {{
                ...
                .then(function(geojson) {{
                    var features = (geojson.features || []).filter(function(f) {{
                        ...
                        var v = props[p.prop_key];
                        if (v === undefined || v === null) return false;
                        if (p.op === 'contains') {{
                            return String(v).toLowerCase().indexOf(String(p.value).toLowerCase()) >= 0;
                        }}
                        if (p.op === 'eq') {{
                        ...
                        if (p.op === 'gt') {{
                            ...
                            return !isNaN(n) && !isNaN(nv) && n > nv;
                        }}
                        return false;
                    }});
                    var out = {{ type: 'FeatureCollection', features: features }};
                    if (map.getSource(p.result_id)) {{
                        map.getSource(p.result_id).setData(out);
                    }} else {{
                        map.addSource(p.result_id, {{ type: 'geojson', data: out }});
                        var layerType;
                        var paint;
                        if (p.layer_type === 'point') {{
                            layerType = 'circle';
                            paint = {{ 'circle-color': p.color, 'circle-radius': 6 }};
                        }} else if (p.layer_type === 'line') {{
                            layerType = 'line';
                            paint = {{ 'line-color': p.color, 'line-width': 2 }};
                        }} else {{
                            layerType = 'fill';
                            paint = {{ 'fill-color': p.color, 'fill-opacity': 0.35 }};
                        }}
                        map.addLayer({{
                            id: p.result_id,
                            type: layerType,
